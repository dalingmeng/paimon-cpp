...
class PAIMON_EXPORT WriteContext {
 public:
    WriteContext(const std::string& root_path, const std::string& commit_user,
                 ...
                 const std::shared_ptr<Executor>& executor, const std::string& io_temp_directory,
    ...
    const std::string& GetIOTempDirectory() const {
        return io_temp_directory_;
    }
...
class PAIMON_EXPORT WriteContextBuilder {
 public:
    ...
    /// @return Reference to this builder for method chaining.
    WriteContextBuilder& WithIOTempDirectory(const std::string& temp_dir);
