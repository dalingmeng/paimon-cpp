...
TEST_F(DataEvolutionSplitReadTest, TestMatch) {
    std::string path = paimon::test::GetDataDir() + "/orc/append_09.db/append_09";
    ReadContextBuilder context_builder(path);
    context_builder.SetReadSchema({"f0", "_ROW_ID", "_SEQUENCE_NUMBER"});
    ...
    auto split_read = std::make_unique<DataEvolutionSplitRead>(
        /*path_factory=*/nullptr, internal_context, pool_,
        CreateDefaultExecutor(/*thread_count=*/2));

    auto create_data_split =
        [](std::vector<std::shared_ptr<DataFileMeta>>&& metas) -> std::shared_ptr<DataSplit> {
        DataSplitImpl::Builder builder(BinaryRow::EmptyRow(),
                                       /*bucket=*/0, /*bucket_path=*/"", std::move(metas));
        return builder.WithSnapshot(1).IsStreaming(false).RawConvertible(true).Build().value();
    };
    {
        // test empty split
        auto data_split = create_data_split({});
    ...
    {
        // test only one file
        auto meta = CreateDataFileMeta(/*file_name=*/"file1.orc", /*first_row_id=*/std::nullopt,
                                       FileSource::Append());
        auto data_split = create_data_split({meta});
        ASSERT_OK_AND_ASSIGN(bool match_result,
                             split_read->Match(data_split, /*force_keep_delete=*/false));
        ASSERT_FALSE(match_result);
    }
    {
        // test with null first row id
        auto meta1 =
            CreateDataFileMeta(/*file_name=*/"file1.orc", /*first_row_id=*/1, FileSource::Append());
        auto meta2 = CreateDataFileMeta(/*file_name=*/"file2.orc", /*first_row_id=*/std::nullopt,
                                        FileSource::Append());
        auto data_split = create_data_split({meta1, meta2});
        ASSERT_OK_AND_ASSIGN(bool match_result,
                             split_read->Match(data_split, /*force_keep_delete=*/false));
        ASSERT_FALSE(match_result);
    }
    {
        ...
        auto meta1 =
            CreateDataFileMeta(/*file_name=*/"file1.orc", /*first_row_id=*/1, FileSource::Append());
        auto meta2 =
            CreateDataFileMeta(/*file_name=*/"file2.orc", /*first_row_id=*/2, FileSource::Append());
        auto data_split = create_data_split({meta1, meta2});
        ASSERT_OK_AND_ASSIGN(bool match_result,
                             split_read->Match(data_split, /*force_keep_delete=*/false));
        ASSERT_FALSE(match_result);
    ...
    {
        // test with compact file source
        auto meta1 =
            CreateDataFileMeta(/*file_name=*/"file1.orc", /*first_row_id=*/1, FileSource::Append());
        auto meta2 = CreateDataFileMeta(/*file_name=*/"file2.orc", /*first_row_id=*/1,
                                        FileSource::Compact());
        auto data_split = create_data_split({meta1, meta2});
        ASSERT_OK_AND_ASSIGN(bool match_result,
                             split_read->Match(data_split, /*force_keep_delete=*/false));
    ...
    {
        // test match simple
        auto meta1 = CreateDataFileMeta(/*file_name=*/"file1.orc", /*first_row_id=*/100,
                                        FileSource::Append());
        auto meta2 = CreateDataFileMeta(/*file_name=*/"file2.orc", /*first_row_id=*/100,
                                        FileSource::Append());
        auto data_split = create_data_split({meta1, meta2});
        ASSERT_OK_AND_ASSIGN(bool match_result,
                             split_read->Match(data_split, /*force_keep_delete=*/false));
        ASSERT_TRUE(match_result);
        // The force_keep_delete parameter is not used in match, so test both values
        ASSERT_OK_AND_ASSIGN(bool match_result2,
                             split_read->Match(data_split, /*force_keep_delete=*/true));
        ASSERT_TRUE(match_result2);
    }
}
