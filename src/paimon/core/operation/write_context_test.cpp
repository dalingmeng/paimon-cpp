...
TEST(WriteContextTest, TestSimple) {
    ...
    ASSERT_TRUE(ctx->GetIOTempDirectory().empty());
    ASSERT_TRUE(ctx->GetOptions().empty());
    ASSERT_TRUE(ctx->GetFileSystemSchemeToIdentifierMap().empty());
}

TEST(WriteContextTest, TestWithIOTempDirectory) {
    WriteContextBuilder builder("table_root_path", "commit_user_1");

    ASSERT_OK_AND_ASSIGN(auto ctx, builder.WithIOTempDirectory("/tmp").Finish());
    ASSERT_EQ(ctx->GetIOTempDirectory(), "/tmp");
}
